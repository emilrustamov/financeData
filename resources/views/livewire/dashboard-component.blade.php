<div class="container mt-4">
    ...
    @php
        $hasFilters = (!empty($selectedCategories) || !empty($filterProjectIds) || !empty($filterCashIds) || !empty($filterObjectIds));
    @endphp
    @if($hasFilters)
        <div class="mb-3">
            ...
            @foreach($selectedCategories as $catId)
                <span class="badge bg-info text-dark me-2">
                    {{ $allCategories[$catId] ?? '??' }}
                    <a href="#" class="text-dark ms-1" wire:click.prevent="toggleCategoryFilter({{ $catId }})">
            ...
            @foreach($filterProjectIds as $pId)
                <span class="badge bg-warning text-dark me-2">
                    {{ $allProjects[$pId] ?? '??' }}
                    <a href="#" class="ms-1 text-dark" wire:click.prevent="toggleProjectFilter({{ $pId }})">
                        <i class="bi bi-x-circle"></i>
                    </a>
                </span>
            @endforeach
            ...
            @foreach($filterCashIds as $cashId)
                <span class="badge bg-secondary text-light me-2">
                    {{ $allCashes[$cashId] ?? '??' }}
                    <a href="#" class="ms-1 text-light" wire:click.prevent="toggleCashFilter({{ $cashId }})">
                        <i class="bi bi-x-circle"></i>
                    </a>
                </span>
            @endforeach
            ...
            @foreach($filterObjectIds as $objId)
                <span class="badge bg-primary text-light me-2">
                    {{ $allObjects[$objId] ?? '??' }}
                    <a href="#" class="ms-1 text-light" wire:click.prevent="toggleObjectFilter({{ $objId }})">
                        <i class="bi bi-x-circle"></i>
                    </a>
                </span>
            @endforeach
    ...
    <div class="card mb-4 p-3" id="chart1">
        ...
        <p class="text-muted mb-2">Общая сумма: {{ number_format($totalAmount, 2, '.', ' ') }}</p>
    ...
    <!-- Таблица (Период 1) -->
    <h4 id="table1">Таблица (Период 1)</h4>
        ...
        <tbody>
            @forelse($records as $rec)
                <tr>
                    ...
                    <td>
                        @if($rec->project)
                            <a href="#" wire:click.prevent="toggleProjectFilter({{ $rec->project->id }})">
                                {{ $rec->project->title }}
                            </a>
                        @else
                            -
                    ...
                    <td>{{ optional($rec->category)->title ?? '-' }}</td>
                    <td>{{ number_format($rec->amount, 2, '.', ' ') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="6">Нет данных по выбранным фильтрам (и датам)</td>
                </tr>
            @endforelse
        </tbody>
        @if($records->count() > 0)
    ...
    <div class="card mb-4 p-3" id="chart2">
     ...
     <!-- Таблица (Период 2) -->
     <h4 id="table2">Таблица (Период 2)</h4>
     <table class="table table-bordered">
         ...
         <tbody>
             @forelse($compareRecords as $rec2)
                 <tr>
                     ...
                     <td>
                         @if($rec2->project)
                             <a href="#" wire:click.prevent="toggleProjectFilter({{ $rec2->project->id }})">
                                 {{ $rec2->project->title }}
                             </a>
                         @else
                             -
                     ...
                     <td>
                         @if($rec2->object)
                             <a href="#" wire:click.prevent="toggleObjectFilter({{ $rec2->object->id }})">
                                 {{ $rec2->object->title }}
                             </a>
                         @else
                             -
                     ...
                     <td>{{ optional($rec2->category)->title ?? '-' }}</td>
                     <td>{{ number_format($rec2->amount, 2, '.', ' ') }}</td>
                 </tr>
             @empty
                 <tr>
                     <td colspan="6">Нет данных во втором периоде</td>
                 </tr>
             @endforelse
         </tbody>
         @if($compareRecords->count() > 0)
             <tfoot>
                 <tr>
                     <th colspan="5" class="text-end">Итого</th>
                     <th>{{ number_format($compareTotalAmount, 2, '.', ' ') }}</th>
                 </tr>
             </tfoot>
         @endif
     </table>
    ...
    @if($cashComparisonChart)
        ...
        @if($cashComparisonTableData)
            <div class="card mb-4 p-3" id="table3">
                <h5>Таблица: Расход по кассам (категория: {{ $allCategories[reset($selectedCategories)] ?? 'Неизвестно' }})</h5>
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>Касса</th>
    ...
    <div class="floating-menu" style="position: fixed; bottom: 10px; right: 10px; background: #fff; border:1px solid #ccc; padding:10px; z-index:999;">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const toggles = [
            { checkboxId: 'toggleChart1', targets: ['chart1','table1'] },
            { checkboxId: 'toggleChart2', targets: ['chart2','table2'] },
            { checkboxId: 'toggleChart3', targets: ['chart3','table3'] },
        ];
        toggles.forEach(({ checkboxId, targets }) => {
            const cb = document.getElementById(checkboxId);
            const stored = localStorage.getItem(checkboxId);
            if(stored === 'false') {
                cb.checked = false;
                targets.forEach(id => document.getElementById(id)?.style.setProperty('display','none'));
            }
